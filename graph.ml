exception Cycle
	...
	let has_cycle g = 
	  ...
	  | NotVisited -> ( 
	    n.n_mark <- InProgress;
	    List.iter visit n.n_link_to; (*visit all neighbouring nodes*)
	    n.n_mark <- Visited)
	  in
	  try
	    clear_marks g;
	...
	let topological g =
	  let rec tsort stack n =
	    match n.n_mark with
	    | InProgress -> raise Cycle
	    | Visited -> stack
	    | NotVisited -> (
	      n.n_mark <- InProgress;
	      let tmp = List.fold_left tsort [] n.n_link_to in
	      n.n_mark <- Visited;
	      n.n_label :: (List.append tmp stack)
	    )
	  in
	  try
	    (*Run tsort with empty stack and root of g*)
	    clear_marks g;
	    List.fold_left tsort [] (find_roots g)
	  with Cycle -> []